...
このMVPでは、AIを単発の文章生成器として使うのではなく、変更依頼の整理、DNSリスク評価、承認文生成、実行手順生成、ロールバック手順生成、証跡レポート生成という複数の役割に分け、順番に処理するAgent pipelineとして構成した。
...
ChangeProof Agent のソリューションは、実作業の自動化ではなく、作業前後に必要な判断材料と証跡をAIエージェントで作ることだ。DNS変更リクエストを入力すると、AIがリスク、未確認事項、承認文、実行手順、ロールバック手順、作業後報告書ドラフトを生成し、Evidence Bundleとして保存する。

つまり、AIにDNSを変更させるのではなく、DNS変更を説明可能な業務プロセスに寄せるためのツールとして作った。
...
だから、AIにDNSを変更させるのではなく、AIに「作業前後の判断材料と証跡」を作らせることにした。自分の結論としては、AIエージェントの使いどころは、いきなり本番操作を自動化するより、このへんの準備・説明・記録に置く方が現実的だと思う。
...
この線引きを明確にすることで、AIを危険な自動操作に使うのではなく、判断材料と証跡作成に使う設計にした。
...
Zip deployでは、publish outputをアップロードしたつもりでもQuickDeployがOryx buildを走らせて、`.csproj`を検出できずに落ちた。これはApp Settingsでserver-side buildを切ることで回避した。

最後に、Azure OpenAI endpointも少しはまった。Foundry UIには `/openai/v1` 付きのendpointが出ることがあるが、Semantic KernelのAzure OpenAI connectorではresource root形式を期待する。アプリ側では `/openai` 以降を正規化するようにした。
...
今回の範囲では、「DNS変更を実行しない」「AI出力を構造化して保存する」「人間承認を状態遷移として残す」という線だけは崩さないようにした。
...
共通しているのは、AIに本番操作を任せるのではなく、人間が責任を持って作業するための判断材料と証跡を揃えることだ。
...
ChangeProof AgentはまだMVPだが、少なくとも「AIに危険な作業を実行させず、判断と証跡を作らせるAIエージェント」という方向性は、自分の中では手応えがあった。
